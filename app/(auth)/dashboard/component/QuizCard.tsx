      ...
      <CardContent className="flex flex-1 flex-col gap-2 px-5 py-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex gap-1">
            ...
            <span
              title={quiz._raw?.isPublic ? "Public" : "Private"}
              className="rounded-lg border border-yellow-200 bg-yellow-50 px-2 py-0.5 text-xs font-bold text-yellow-700 uppercase dark:border-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-500">
        ...
        {/* Judul */}
        <Link
          className="line-clamp-1 cursor-pointer text-lg font-bold text-zinc-800 dark:text-zinc-300"
          title={quiz.title}
          href={`/detail/${quiz.id}`}>
          {quiz.title}
        </Link>
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-3">
          <div className="flex min-w-0 flex-col gap-1 text-xs font-medium text-zinc-700 dark:text-zinc-400">
            <div className="flex shrink-0 items-center gap-1.5">
              <CircleQuestionMark size={14} className="shrink-0 text-green-500" />
              <span className="truncate">{quiz.questions} Soal</span>
            </div>
            <div className="flex shrink-0 items-center gap-1.5">
              <Play size={14} className="shrink-0 text-yellow-500" />
